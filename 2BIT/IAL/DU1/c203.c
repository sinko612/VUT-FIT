...
void Queue_Init( Queue *queue ) {
	...
	//overenie ci nedostane queue == NULL
	if(queue == NULL){
		Queue_Error(QERR_INIT);
		return;
	}
	else if (queue->array != NULL){ //nastava mozny pripad, ze budeme chciet inicializovat uz existujucu frontu
		// inicializacia fronty
		queue->firstIndex = 0;
		queue->freeIndex = 0;

		// nastavenie hodnot prvkov pola na *
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	...
	// overenie, ci nie je nahodou fronta prazdna
	if(Queue_IsEmpty(queue)){
		Queue_Error(QERR_FRONT);
		return;
	}
	else{
		// predanie hodnoty prveho prvku pola
...
void Queue_Remove( Queue *queue ) {
	...
	// overenie ci je fronta prazdna
	if(Queue_IsEmpty(queue)){
		Queue_Error(QERR_REMOVE);
		return;
	}
	else{
		// prepisanie prveho indexu na nasledujuci
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	...
	if(Queue_IsEmpty(queue)){
		...
		// vratenie prveho znaku fronty
		Queue_Front(queue, dataPtr);
		// odstranenie znaku zo zaciatku fronty
		Queue_Remove(queue);
	}
...
void Queue_Enqueue( Queue *queue, char data ) {
	...
	if(Queue_IsFull(queue)){
		...
		// vlozenie znaku do fronty
		queue->array[queue->freeIndex] = data;
		// posunutie freeIndexu na nasledujuce volne miesto
